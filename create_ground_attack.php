...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($character_name_err) && empty($jab_1_err) && empty($jab_2_err) && empty($jab_3_err) && empty($rapid_jab_err) && empty($forward_tilt_err) && empty($up_tilt_err) 
		&& empty($down_tilt_err) && empty($forward_smash_err) && empty($up_smash_err)&& empty($down_smash_err) && empty($dash_attack_err)){
        // Prepare an insert statement
        $sql = "INSERT INTO ground_attacks (character_name, jab_1, jab_2, jab_3, rapid_jab, forward_tilt, up_tilt, down_tilt, forward_smash, up_smash, down_smash, dash_attack) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "siiiiiiiiiii", $param_character_name, $param_jab_1, $param_jab_2, $param_jab_3, 
			$param_rapid_jab, $param_forward_tilt, $param_up_tilt, $param_down_tilt, $param_forward_smash, $param_up_smash, $param_down_smash, $param_dash_attack);
